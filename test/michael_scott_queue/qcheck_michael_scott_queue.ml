module Qcheck_ms_queue (Queue : Ms_queues.MS_queue_tests) = struct
...
let () =
  ...
  let module Safe = Qcheck_ms_queue (Ms_queues.Michael_scott_queue) in
  let name = "safe" in
  let safe_tests =
    [
  ...
  in
  let module Unsafe = Qcheck_ms_queue (Ms_queues.Michael_scott_queue_unsafe) in
  let name = "unsafe" in
  let unsafe_tests =
    [
  ...
  in
  Alcotest.run "Michael_scott_queue" (safe_tests @ unsafe_tests)
